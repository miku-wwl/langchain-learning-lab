"""Discover the loaded, tool-capable Foundry Local model for this chapter."""
...
def local_chat_config(alias: str = "phi-4-mini") -> tuple[str, str]:
    ...
    if not model_id:
        ...
        selected = next((item for item in loaded if item.get("alias") == alias), None)
    ...
    if not base_url.startswith("http://127.0.0.1:") and not base_url.startswith("http://localhost:"):
        raise RuntimeError("Only a loopback local model endpoint is allowed")
    return base_url, model_id


def create_local_model(*, alias: str = "phi-4-mini", max_tokens: int = 256) -> ChatOpenAI:
    base_url, model_id = local_chat_config(alias)
    return ChatOpenAI(
        model=model_id, base_url=base_url, api_key="local",
        temperature=0, max_tokens=max_tokens, max_retries=0, timeout=120,
    )
